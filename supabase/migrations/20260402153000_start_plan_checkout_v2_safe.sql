...
CREATE OR REPLACE FUNCTION public.start_plan_checkout_v2(
  p_plan_slug TEXT,
...
BEGIN
  ...
  SELECT
    ...
    s.payment_expires_at
  FROM public.store_subscriptions s
  WHERE s.id = v_inserted_id;
END;
$$;

REVOKE ALL ON FUNCTION public.start_plan_checkout_v2(TEXT, public.plan_billing_cycle, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.start_plan_checkout_v2(TEXT, public.plan_billing_cycle, TEXT) TO authenticated;
